...
function Resolve-PythonInvocation {
  ...
  foreach ($candidate in ($candidates | Select-Object -Unique)) {
    if (Test-Path $candidate) {
      return Quote-CmdValue (Resolve-Path $candidate).Path
    }
    ...
    if ($command -and $command.Source) {
      ...
      return Quote-CmdValue $command.Source
    }
  }
  $py = Get-Command "py" -ErrorAction SilentlyContinue
  if ($py -and $py.Source) {
    return (Quote-CmdValue $py.Source) + " -3"
  }
  $codexPython = Join-Path $env:USERPROFILE ".cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe"
  if (Test-Path $codexPython) {
    return Quote-CmdValue $codexPython
  }
  return $null
}
...
if (-not (Test-TcpPort $HostAddress $WebPort)) {
  $pythonInvocation = Resolve-PythonInvocation $PythonCommand
  if (-not $pythonInvocation) {
    throw "Python was not found. Install Python 3.11+ or pass -PythonCommand C:\path\to\python.exe."
  }
...
cd /d $Root
$pythonInvocation -m lookup_tool.cli serve --host $HostAddress --port $WebPort >> "$webLog" 2>> "$webErr"
"@
