...
- 建立 Windows 托盘 EXE 启动器，负责自动启动 Docker Desktop、等待 Docker daemon、执行 Docker Compose 并打开 RAGFlow。
- 运行合同测试。
...
- Windows 一键启动器位于 `launcher/LocalMathRAGFlow`，构建脚本位于 `scripts/build-launcher.ps1`。
